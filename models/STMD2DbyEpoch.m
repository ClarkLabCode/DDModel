function a = STMD2DbyEpoch(dataPath,varargin)
...
% The model architecutre is based on Wiederman et al. (2013) JNS.
...
for ee = 1:numEpochs
    ...
    sizeT = size(S,3);
    % show stimulus
    subplot(numEpochs,nStep,nStep*(ee-1)+1); plot(permute(S(round(sizeX/2),:,:),[3,2,1])); title('Stimulus');
    drawnow;
    ...
    ROFF = DoGrect(-RHP.*(RHP<0),kerSizes,kerWeights,filterOrder);
    
    subplot(numEpochs,nStep,nStep*(ee-1)+3); 
    plot(permute(RON(round(sizeX/2),:,:),[3,2,1]));  title('Surround supression (ON)');
    subplot(numEpochs,nStep,nStep*(ee-1)+4); 
    plot(permute(ROFF(round(sizeX/2),:,:),[3,2,1])); title('Surround supression (OFF)');

    %% 3. Delay OFF input
    kerTdelay = timeX.* exp(-timeX/tau_d)/tau_d/tau_d;
    kerTdelay = kerTdelay / sqrt(sum(kerTdelay.^2)/frameRate); % unit l2 norm
    
    ROFFD = convn(ROFF,permute(kerTdelay,[3,1,2]),'full')/frameRate;
    ROFFD = ROFFD(:,:,1:size(ROFF,3)); % truncate
    
    subplot(numEpochs,nStep,nStep*(ee-1)+5); 
    plot(permute(ROFFD(round(sizeX/2),:,:),[3,2,1])); title('Delayed OFF');
    
    %% 4. Multiplication %%
    % adaptation state = low-pass filtered version of stimulus
    kerTout = timeX.* exp(-timeX/tau)/tau/tau;
    kerTout = kerTout / sqrt(sum(kerTout.^2)/frameRate); % unit l2 norm
    Rout = convn(RON.*ROFFD,permute(kerTout,[3,1,2]),'full')/frameRate;
    Rout = Rout(:,:,1:size(ROFFD,3));
    subplot(numEpochs,nStep,nStep*(ee-1)+6); 
    plot(permute(Rout(round(sizeX/2),:,:),[3,2,1])); title('Output');
